<?php
/**
 * POI Entur Block Template
 * Single POI selector for Entur bus/transit stops with live departures
 *
 * @package Placy
 * @since 1.0.0
 */
...
$block_id = 'poi-entur-' . $block['id'];
$class_name = 'poi-entur-block';
...
<div id="<?php echo esc_attr( $block_id ); ?>" class="<?php echo esc_attr( $class_name ); ?> w-full mb-6">
    <?php if ( $poi_item ) : ?>
        ...
        $entur_stopplace_id = get_field( 'entur_stopplace_id', $poi_item->ID );
        $entur_quay_id = get_field( 'entur_quay_id', $poi_item->ID );
        $entur_transport_mode = get_field( 'entur_transport_mode', $poi_item->ID );
        $entur_group_by_direction = get_field( 'entur_group_by_direction', $poi_item->ID );
        $entur_line_filter = get_field( 'entur_line_filter', $poi_item->ID );
        $show_live_departures = get_field( 'show_live_departures', $poi_item->ID );
        ...
        <div <?php if ( $is_in_chapter ) echo 'data-chapter-poi-entur="true"'; ?>>
    ...
    <?php else : ?>
        <p class="p-8 text-center text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            <?php _e( 'Ingen POI valgt. Vennligst velg en Entur-stoppeplass i blokkinnstillingene.', 'placy' ); ?>
        </p>
    <?php endif; ?>
</div>
