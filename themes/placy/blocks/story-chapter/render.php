<?php
/**
 * Story Chapter Block Template
 *
 * Renders chapter section with front section and mega-modal.
...
if ( $post ) {
    ...
    if ( get_post_type( $post->ID ) === 'project' ) {
        ...
        // For theme-story or story, get parent project
...
// Block classes
$block_classes = array( 'story-chapter', 'narrative-section' );
...
if ( $is_preview && ! $has_content ) : ?>
    <div class="story-chapter-preview p-8 bg-gray-100 border-2 border-dashed border-gray-300 rounded-lg text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-700 mb-2">Story Chapter</h3>
        <p class="text-gray-500 mb-4">Configure this chapter:</p>
        <div class="text-sm text-gray-600">
            <p class="mb-2"><strong>Front Section:</strong> Add title, ingress and highlighted places</p>
            <p><strong>Mega-modal:</strong> Select a Theme Story for rich content</p>
        </div>
    </div>
<?php
    return;
...
>
    ...
    <div class="narrative-header mx-auto px-6 py-12">
    ...
    <?php if ( ! empty( $highlighted_points_data ) || $show_cta_button ) : ?>
        <div class="poi-cards-grid max-w-6xl px-6 pb-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                foreach ( $display_points as $index => $point ) : 
                    // Calculate travel times based on distance from project
                    // placy_calculate_distance() returns meters, convert to km
                    $distance_m = function_exists( 'placy_calculate_distance' ) 
                        ? placy_calculate_distance( $project_lat, $project_lng, $point['lat'], $point['lng'] )
                        : 0;
                    $distance_km = $distance_m / 1000;
                    $walk_time = max( 1, round( ( $distance_km / 5 ) * 60 ) );   // 5 km/h
                    $bike_time = max( 1, round( ( $distance_km / 15 ) * 60 ) );  // 15 km/h
                    $car_time = max( 1, round( ( $distance_km / 40 ) * 60 ) );   // 40 km/h
                    
                    $poi_times = wp_json_encode( array(
                        'walk' => (int) $walk_time,
                        'bike' => (int) $bike_time,
                        'car'  => (int) $car_time,
                    ) );
                ...
                ?>
                    <div class="poi-card bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow cursor-pointer relative"
                         ...
                         data-poi-times='<?php echo esc_attr( $poi_times ); ?>'
                         onclick="openChapterMegaModal('<?php echo esc_attr( $chapter_id ); ?>', <?php echo esc_js( $point['id'] ); ?>)">
                        
                        <!-- Travel time badge (top-right) -->
                        <div class="poi-travel-badge absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-full px-2.5 py-1 text-xs font-medium text-gray-700 shadow-sm z-10 flex items-center gap-1">
                            <span class="poi-travel-icon">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path d="M13 6a2 2 0 100-4 2 2 0 000 4zm-1 2L8 14l-2-1m6-5l4 4m-4-4l-4 4m0 6l2-2m8-4l-4 4-2-2"/>
                                </svg>
                            </span>
                            <span class="poi-travel-time"><?php echo esc_html( $walk_time ); ?></span>
                            <span class="poi-travel-label">min walk</span>
                        </div>
    ...
    <?php
        // Get property logo and label from project
        $property_logo = '';
        $property_label = '';
        if ( $project_id ) {
            $property_logo = get_field( 'property_logo', $project_id );
            $property_label = get_field( 'property_label', $project_id );
        }
        ...
        $modal_data = array(
            ...
            'propertyLogo'       => $property_logo,       // Project logo URL
            'propertyLabel'      => $property_label,      // Project/property name
        );
