                    ...
                    <div class="poi-card-content flex gap-4 p-4">
                        ...
                        <div class="flex-1 min-w-0 flex flex-col">
                            <div class="flex items-start justify-between">
                                ...
                                $place_details_cache = get_field( 'place_details_cache', $poi->ID );
                                $place_details = ! empty( $place_details_cache ) ? json_decode( $place_details_cache, true ) : array();
                                $has_contact_info = ! empty( $place_details['website'] ) || ! empty( $place_details['phone'] );
                                
                                if ( $has_contact_info ) : ?>
                                    <div class="poi-contact-info flex items-center gap-4 mt-2 text-xs text-gray-600">
                                        <?php if ( ! empty( $place_details['website'] ) ) : ?>
                                            <a href="<?php echo esc_url( $place_details['website'] ); ?>" target="_blank" rel="noopener" class="flex items-center gap-1 hover:text-blue-600 transition-colors">
                                        ...
                                        <?php endif; ?>
                                        <?php if ( ! empty( $place_details['phone'] ) ) : ?>
                                            <a href="tel:<?php echo esc_attr( str_replace( ' ', '', $place_details['phone'] ) ); ?>" class="flex items-center gap-1 hover:text-blue-600 transition-colors">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                                                </svg>
                                                <span><?php echo esc_html( $place_details['phone'] ); ?></span>
                                            </a>
                                        <?php endif; ?>
                                    </div>
                                <?php endif; ?>
