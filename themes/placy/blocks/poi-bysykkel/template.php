<?php
/**
 * POI Bysykkel Block Template
 * Single POI selector for Trondheim Bysykkel stations with live availability
 *
 * @package Placy
 * @since 1.0.0
 */
...
$block_id = 'poi-bysykkel-' . $block['id'];
$class_name = 'poi-bysykkel-block';
...
<div id="<?php echo esc_attr( $block_id ); ?>" class="<?php echo esc_attr( $class_name ); ?> w-full mb-6">
    <?php if ( $poi_item ) : ?>
        ...
        $bysykkel_station_id = get_field( 'bysykkel_station_id', $poi_item->ID );
        $show_bike_availability = get_field( 'show_bike_availability', $poi_item->ID );
        ...
        // Add "Trondheim Bysykkel:" prefix if not already present
        if ( strpos( $poi_name, 'Trondheim Bysykkel:' ) !== 0 ) {
            $display_name = 'Trondheim Bysykkel: ' . $poi_name;
        } else {
            $display_name = $poi_name;
        }
        ?>
        
        <div <?php if ( $is_in_chapter ) echo 'data-chapter-poi-bysykkel="true"'; ?>>
    ...
    <?php else : ?>
        <p class="p-8 text-center text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            <?php _e( 'Ingen POI valgt. Vennligst velg en Bysykkel-stasjon i blokkinnstillingene.', 'placy' ); ?>
        </p>
    <?php endif; ?>
</div>
